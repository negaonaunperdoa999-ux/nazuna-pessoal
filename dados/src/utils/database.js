...
const setGroupRental = (groupId, durationDays) => {
  ...
  if (existsNow) {
    ...
    console.log('?? N�O persistiu imediatamente');
  }
...
function ensureEconomyDefaults(econ) {
  ...
  // === MIGRA��O AUTOM�TICA DE USU�RIOS ===
  ...
  if (!econ.lottery.tickets || typeof econ.lottery.tickets !== 'object' || Array.isArray(econ.lottery.tickets)) {
    econ.lottery.tickets = {};
    changed = true;
  }

  if (!Array.isArray(econ.lottery.winners)) {
    econ.lottery.winners = [];
    changed = true;
  }

  if (typeof econ.lottery.drawInterval !== 'number' || econ.lottery.drawInterval <= 0) {
    econ.lottery.drawInterval = 86400000;
    changed = true;
  }

  if (typeof econ.lottery.ticketPrice !== 'number' || econ.lottery.ticketPrice <= 0) {
    econ.lottery.ticketPrice = 100;
    changed = true;
  }
...
// ═══════════════════════════════════════════════════════════════════
// FUN��ES DE LEVELING COM PROTE��O
// ═══════════════════════════════════════════════════════════════════
